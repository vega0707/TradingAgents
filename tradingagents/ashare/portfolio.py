...
def render(port: dict, capital: float = 1_000_000) -> str:
    ...
    # 资产配置（永续框架，用户 2026-09-18："回撤我觉得是整体组合来看的"）。
    # bt_permanent 实测（2016-12~2026-09，9.4 年，含 2018/2022/2023 三场熊市）：
    #   100% 股票 年化 +3.2% / 回撤 45.6% ｜ 永续 25×4 年化 +6.6% / 回撤 9.2%
    # 股票腿用我们的精选（红利+质量：熊市年比沪深300 抗跌 2-5pp）。
    # 单票风格闸（PB>4）保留在选择逻辑里，但不再当作组合层否决——它会把对冲腿砍掉。
    if equity_total > 0:
        _tgt = {"股票": 0.25, "长债": 0.25, "黄金": 0.25, "现金": 0.25}
        _now = {"股票": equity / equity_total, "长债": 0.0, "黄金": 0.0,
                "现金": cash / equity_total}
        _leg = equity_total * 0.25 / 10000
        lines += ["## 资产配置（永续框架 25%×4）"]
        for _k in ("股票", "长债", "黄金", "现金"):
            _gap = (_now[_k] - _tgt[_k]) * equity_total
            lines.append(f"- {_k}：目标 {_tgt[_k]*100:.0f}%（{equity_total*_tgt[_k]/10000:.2f}万）"
                         f" ｜ 现状 {_now[_k]*100:.0f}%（{_now[_k]*equity_total/10000:.2f}万）"
                         f" ｜ 差 {_gap/10000:+.2f}万")
        lines += [f"- 待补三腿各 {_leg:.2f}万：长债（十年国债ETF 511260）＋黄金（黄金ETF 518880）"
                  f"＋现金（货币基金/511990）",
                  "- 股票超配优先用**新增资金**补其他腿，避免一次性割肉；若必须减持，"
                  "先动已给减仓信号的票", ""]
