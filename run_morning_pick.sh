...
RC=$?
if [[ $RC -ne 0 ]]; then
  # 异常失败（LLM 超时/数据源挂）→ 必须可见，不能静默（2026-09-11 教训）
  echo "[$DATE] 异常失败 rc=$RC → 推送失败告警" >> "$LOG"
  printf '# ⚠️ 早盘推荐任务失败 · %s\n\nLLM/数据源异常，本次未生成推荐。\n原因见 logs/morning-%s.log（尾行）。\n' "$DATE" "$DATE" > /tmp/morning-fail.md
  scp -i "$KEY" -q /tmp/morning-fail.md ubuntu@124.221.95.205:/tmp/morning-fail.md 2>>"$LOG"
  ssh -i "$KEY" ubuntu@124.221.95.205 "cd /home/ubuntu/TradingAgents && set -a && source .env && set +a && \
    ...
    \$NODE \$BIN message send --channel feishu --account \"\$FEISHU_ACCOUNT\" --target \"\$FEISHU_TARGET\" -m \"\$(cat /tmp/morning-fail.md)\"" >>"$LOG" 2>&1 \
    && echo "[$DATE] 已发失败告警" >> "$LOG" || true
  exit 0
fi
if [[ ! -s /tmp/ashare-morning.md ]]; then
  echo "[$DATE] 输出为空 → 正常静默（快讯不足）" >> "$LOG"
  exit 0
fi
# 无候选深析段则说明快讯不足，不推
if ! grep -q "候选深析" /tmp/ashare-morning.md; then
  echo "[$DATE] 无深析候选 → 正常静默（快讯不足）" >> "$LOG"
  exit 0
fi
